---
layout: Conceptual
title: Handling Screen Savers - Win32 apps | Microsoft Learn
canonicalUrl: https://learn.microsoft.com/en-us/windows/win32/lwef/screen-saver-library
...
locale: en-us
document_id: cb5147ce-8808-9cc3-5f54-b8ea40b3bd21
document_version_independent_id: e7b495c0-ed82-b645-5712-4d684d06f9b8
updated_at: 2025-03-13T18:20:00.0000000Z
original_content_git_url: https://github.com/MicrosoftDocs/win32-pr/blob/live/desktop-src/lwef/screen-saver-library.md
gitcommit: https://github.com/MicrosoftDocs/win32-pr/blob/fd5b414bbed38784ccf90c12b7bbe416fe7710ed/desktop-src/lwef/screen-saver-library.md
...
word_count: 2280
asset_id: lwef/screen-saver-library
moniker_range_name: 
monikers: []
item_type: Content
source_path: desktop-src/lwef/screen-saver-library.md
cmProducts:
...
platformId: 5e202b5a-2581-07a7-3d3e-a0a3686fc6c8
---
...
- The active application receives the [WM_SYSCOMMAND](../menurc/wm-syscommand) message with the *wParam* parameter set to the SC\_SCREENSAVE value, but it does not pass the message to the [DefWindowProc](/en-us/windows/win32/api/winuser/nf-winuser-defwindowproca) function.
...
| WM\_DESTROY | Destroy the timers created when the application processed the [WM_CREATE](../winmsg/wm-create) message. Perform any additional required cleanup. |

[**ScreenSaverProc**](/en-us/windows/desktop/api/scrnsave/nf-scrnsave-screensaverproc) passes unprocessed messages to the screen saver library by calling the [**DefScreenSaverProc**](/en-us/windows/desktop/api/scrnsave/nf-scrnsave-defscreensaverproc) function. The following table describes how this function processes various messages.
...
The second required function in a screen saver module is [**ScreenSaverConfigureDialog**](/en-us/windows/desktop/api/scrnsave/nf-scrnsave-screensaverconfiguredialog). This function displays a dialog box that enables the user to configure the screen saver (an application must provide a corresponding dialog box template). Windows displays the configuration dialog box when the user selects the **Setup** button in the Control Panel's Screen Saver dialog box.

The third required function in a screen saver module is [**RegisterDialogClasses**](/en-us/windows/desktop/api/scrnsave/nf-scrnsave-registerdialogclasses). This function must be called by all screen saver applications. However, applications that do not require special windows or custom controls in the configuration dialog box can simply return **TRUE**. Applications requiring special windows or custom controls should use this function to register the corresponding window classes.
...
At intervals ranging from 1 through 10 seconds, the application in this example repaints the screen with one of four colors: white, light gray, dark gray, and black. The application paints the screen each time it receives a [WM_TIMER](../winmsg/wm-timer) message. The user can adjust the interval at which this message is sent by selecting the application's configuration dialog box and adjusting a single horizontal scroll bar.
...
The following example shows the [**ScreenSaverConfigureDialog**](/en-us/windows/desktop/api/scrnsave/nf-scrnsave-screensaverconfiguredialog) function found in the sample application.
...
In addition to providing the dialog box template and supporting the [**ScreenSaverConfigureDialog**](/en-us/windows/desktop/api/scrnsave/nf-scrnsave-screensaverconfiguredialog) function, an application must also support the [**RegisterDialogClasses**](/en-us/windows/desktop/api/scrnsave/nf-scrnsave-registerdialogclasses) function. This function registers any nonstandard window classes required by the screen saver. Because the sample application used only standard window classes in its dialog box procedure, this function simply returns **TRUE**, as in the following example:
...
Each screen saver must support a window procedure named [**ScreenSaverProc**](/en-us/windows/desktop/api/scrnsave/nf-scrnsave-screensaverproc). Like most window procedures, **ScreenSaverProc** processes a set of specific messages and passes any unprocessed messages to a default procedure. However, instead of passing them to the [DefWindowProc](/en-us/windows/win32/api/winuser/nf-winuser-defwindowproca) function, **ScreenSaverProc** passes unprocessed messages to the [**DefScreenSaverProc**](/en-us/windows/desktop/api/scrnsave/nf-scrnsave-defscreensaverproc) function. Another difference between **ScreenSaverProc** and a normal window procedure is that the handle passed to **ScreenSaverProc** identifies the entire desktop rather than a client window. The following example shows the **ScreenSaverProc** window procedure for the sample screen saver.
...
The [**ScreenSaverProc**](/en-us/windows/desktop/api/scrnsave/nf-scrnsave-screensaverproc) and [**ScreenSaverConfigureDialog**](/en-us/windows/desktop/api/scrnsave/nf-scrnsave-screensaverconfiguredialog) functions must be exported in the application's module-definition file; [**RegisterDialogClasses**](/en-us/windows/desktop/api/scrnsave/nf-scrnsave-registerdialogclasses) should not be exported, however. The following example shows the module-definition file for the sample application.
...
The configuration dialog box for a screen saver typically includes a **Help** button. Screen saver applications can check for the Help button identifier and call the [**WinHelp**](/en-us/windows/desktop/api/winuser/nf-winuser-winhelpa) function in the same way Help is provided in other Windows-based applications.